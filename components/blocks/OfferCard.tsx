...
export function OfferCard({
  ...
  return (
    <section
      id={id}
      className={`py-16 ${variant === "surface" ? "bg-surface" : "bg-bg"}`}
    >
      <div className="mx-auto max-w-[var(--width-wide)] px-6">
        <div className="mx-auto max-w-2xl">
          <h3 className="gradient-text font-serif text-2xl leading-tight tracking-tight">
            {title}
          </h3>
          <p className="mt-3 text-lg text-muted">{hook}</p>

          {itemsLabel && (
            <p className="mt-6 text-sm font-medium text-text">{itemsLabel}</p>
          )}
